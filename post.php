...
$back = 'index.php'; // default
if (isset($_GET['from'])) {
    if ($_GET['from'] === 'profile') $back = 'profile.php';
    elseif ($_GET['from'] === 'explore') $back = 'explore.php';
    elseif ($_GET['from'] === 'index') $back = 'index.php';
}
...
<body>
  ...
  <div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
    <?php if (!empty($post['Profile_pic'])): ?>
      <img src="<?= htmlspecialchars($post['Profile_pic']) ?>" style="width:32px; height:32px; border-radius:50%; object-fit:cover;">
    <?php else: ?>
      <span style="font-size:20px;">👤</span>
    <?php endif; ?>
    <h3 style="margin:0;"><?= htmlspecialchars($post['Username']) ?></h3>
  </div>
